 ...
 * GET /check/:username
 */
router.get('/check/:username', async (req, res): Promise<void> => {
  try {
...
router.get('/:username', async (req, res): Promise<void> => {
  try {
    ...
    // Check cache first
    const cacheKey = CacheService.KEYS.USER_PROFILE_BY_USERNAME(normalizedusersname);
    ...
    const reviewStats = user.reviews_reviews_revieweeIdTousers.length > 0 ? {
      ...
      averageRating: user.reviews_reviews_revieweeIdTousers.reduce((sum: number, review: any) => sum + review.rating, 0) / user.reviews_reviews_revieweeIdTousers.length,
      ratingDistribution: {
        5: user.reviews_reviews_revieweeIdTousers.filter((r: any) => r.rating === 5).length,
        4: user.reviews_reviews_revieweeIdTousers.filter((r: any) => r.rating === 4).length,
        3: user.reviews_reviews_revieweeIdTousers.filter((r: any) => r.rating === 3).length,
        2: user.reviews_reviews_revieweeIdTousers.filter((r: any) => r.rating === 2).length,
        1: user.reviews_reviews_revieweeIdTousers.filter((r: any) => r.rating === 1).length,
      }
    } : null;
    ...
    await cacheService.set(cacheKey, publicProfile, CacheService.TTL.USER_PROFILE);
